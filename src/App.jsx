...
function App() {
  ...
  const [viewMode, setViewMode] = useState(localStorage.getItem('viewMode') || 'responsive'); // 'responsive' or 'desktop'
  ...
  const toggleViewMode = () => {
    setViewMode(prev => prev === 'responsive' ? 'desktop' : 'responsive');
  };
  ...
  return (
    <div 
      className={`min-h-screen bg-white text-pepe-black selection:bg-pepe-pink selection:text-white ${isRTL ? 'rtl' : 'ltr'} ${viewMode === 'desktop' ? 'min-w-[1280px] overflow-x-auto' : ''}`} 
      dir={isRTL ? 'rtl' : 'ltr'}
    >
      {/* View Mode Toggle Button (Mobile Only Floating) */}
      <div className="fixed bottom-6 right-6 z-[80] md:hidden">
        <button
          onClick={toggleViewMode}
          className="bg-pepe-black text-white p-4 rounded-full shadow-2xl border-2 border-white flex items-center space-x-2 space-x-reverse"
        >
          <Globe size={20} />
      ...
      >
        <div className="text-center space-y-8 py-10">
          ...
          >
            <img src={ASSETS.HERO_CHARACTER_IMAGE} alt="Welcome" className="w-full h-full object-contain" />
          </motion.div>
          <div className="space-y-4">
            ...
            <p className="text-2xl font-bold text-gray-700 leading-relaxed max-w-2xl mx-auto">
              {t('hero.desc')}
            </p>
          </div>
          <div className="flex justify-center gap-4 pt-6">
            <Button variant="primary" className="px-10 py-4 text-xl shadow-[6px_6px_0_0_#000]" onClick={() => setActiveModal(null)}>
              {t('hero.join_presale')}
            </Button>
          </div>
        </div>
      ...
      <Modal 
        ...
        headerColor="bg-pepe-green"
      >
        <div className="space-y-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
            <div className="relative rounded-3xl border-4 border-pepe-black overflow-hidden shadow-[8px_8px_0_0_#000]">
              <img src={ASSETS.HERO_CHARACTER_IMAGE} alt="Tokenomics" className="w-full h-full object-cover" />
            </div>
            <div className="space-y-6">
              <h4 className="text-3xl font-black uppercase italic text-pepe-green">{t('tokenomics.title')}</h4>
              <p className="text-lg font-bold text-gray-700 leading-relaxed">
                {t('tokenomics.desc_detailed', 'Pepe Wife features a sustainable economic model designed for long-term growth and community rewards. Our zero-tax policy ensures maximum efficiency for traders.')}
              </p>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gray-50 p-4 rounded-2xl border-2 border-pepe-black/10">
                  <div className="text-xs font-black text-gray-400 uppercase">{t('tokenomics.zero_tax')}</div>
                  <div className="text-xl font-black text-pepe-green">0% BUY / 0% SELL</div>
                </div>
                <div className="bg-gray-50 p-4 rounded-2xl border-2 border-pepe-black/10">
                  <div className="text-xs font-black text-gray-400 uppercase">{t('tokenomics.total_supply_label', 'Total Supply')}</div>
                  <div className="text-xl font-black text-pepe-pink">1 BILLION</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left rtl:text-right border-collapse">
              ...
              <tbody>
                {[
                  { cat: t('tokenomics.category_presale', 'Presale'), pct: '40%', amt: '400,000,000' },
                  { cat: t('tokenomics.category_liquidity', 'Liquidity'), pct: '30%', amt: '300,000,000' },
                  { cat: t('tokenomics.category_staking', 'Staking'), pct: '15%', amt: '150,000,000' },
                  { cat: t('tokenomics.category_team', 'Team'), pct: '10%', amt: '100,000,000' },
                  { cat: t('tokenomics.category_marketing', 'Marketing'), pct: '5%', amt: '50,000,000' },
                ].map((row, i) => (
                  <tr key={i} className="border-4 border-pepe-black font-bold">
          ...
          <div className="text-center text-xl font-black uppercase italic py-4 bg-pepe-yellow/10 border-4 border-pepe-black rounded-2xl shadow-[4px_4px_0_0_#000]">
      ...
      <Modal
        ...
        onClose={() => setActiveModal(null)}
        title={t('roadmap.title_roadmap')}
        headerColor="bg-pepe-pink"
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {[1, 2, 3, 4].map(phase => (
            <div key={phase} className="bg-white p-8 rounded-[2rem] border-4 border-pepe-black shadow-[8px_8px_0_0_#000] hover:shadow-[12px_12px_0_0_#000] transition-all hover:-translate-y-1">
              ...
              <h3 className="text-2xl font-black uppercase italic mb-6 text-pepe-black">{t(`roadmap.phase${phase}_title`)}</h3>
              <ul className="space-y-4">
                {[1, 2, 3, 4].map(item => (
                  <li key={item} className="flex items-start space-x-3 space-x-reverse group">
                    ...
                    <span className="text-gray-700 font-bold text-base leading-tight">
                      {t(`roadmap.phase${phase}_item${item}`)}
                    </span>
                  </li>
                ))}
              </ul>
      ...
      >
        <div className="space-y-6">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="bg-white rounded-[2rem] border-4 border-pepe-black shadow-[6px_6px_0_0_#000] p-8">
              <h4 className="text-2xl font-black uppercase italic text-pepe-black mb-4">{t(`faq.q${i}`)}</h4>
              <p className="text-gray-700 font-bold text-lg leading-relaxed border-t-4 border-pepe-black/5 pt-4">
                {t(`faq.a${i}`)}
              </p>
            </div>
          ))}
      ...
      >
        <div className="space-y-10">
          <div className="text-center space-y-6">
            <h4 className="text-4xl font-black uppercase italic animate-title-gradient">{t('whybuy.title_why')} {t('whybuy.title_pepewife')}</h4>
            <p className="text-xl font-bold text-gray-700 leading-relaxed max-w-3xl mx-auto">
              {t('whybuy.desc')}
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[1, 2, 3].map(i => (
              <div key={i} className="bg-white p-8 rounded-[2rem] border-4 border-pepe-black shadow-[8px_8px_0_0_#000] text-center space-y-4">
                <div className="w-16 h-16 bg-pepe-yellow rounded-2xl border-4 border-pepe-black flex items-center justify-center mx-auto shadow-[4px_4px_0_0_#000]">
                  {i === 1 ? <Rocket size={32} /> : i === 2 ? <Globe size={32} /> : <Shield size={32} />}
                </div>
                <h5 className="text-xl font-black uppercase italic">{t(`whybuy.reason${i}_title`)}</h5>
                <p className="text-sm font-bold text-gray-600">{t(`whybuy.reason${i}_desc`)}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          >
            ...
            <nav className="flex flex-col items-center space-y-6">
              {[
                ...
                <button 
                  key={item.id} 
                  className="text-3xl font-black uppercase hover:text-pepe-pink transition-colors" 
                  onClick={() => {
          ...
          <motion.div initial={{ y: 50, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} className="bg-white/95 backdrop-blur-sm p-6 sm:p-12 rounded-[2rem] sm:rounded-[2.5rem] border-4 border-pepe-black shadow-[8px_8px_0_0_#000] relative overflow-hidden">
            ...
            <div className="space-y-6 sm:space-y-8 max-w-2xl mx-auto">
              ...
              <div className="flex flex-col space-y-4 sm:space-y-6">
                <Button variant="primary" className="w-full py-4 sm:py-6 text-2xl sm:text-3xl shadow-[6px_6px_0px_0px_rgba(10,10,10,1)] sm:shadow-[8px_8px_0px_0px_rgba(10,10,10,1)]">
                  {t('buybox.buy_now')}
                </Button>
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
          <div className="space-y-8">
            <div className="bg-white p-6 sm:p-10 rounded-[2rem] sm:rounded-[2.5rem] border-4 border-pepe-black shadow-[8px_8px_0_0_rgba(10,10,10,1)] lg:shadow-[12px_12px_0px_0px_rgba(10,10,10,1)]">
              ...
              <Button 
                variant="primary" 
                className="w-full mt-10 py-4 text-lg sm:text-xl shadow-[6px_6px_0_0_rgba(10,10,10,1)]"
          ...
          <div className="flex flex-col space-y-6">
            ...
            <div className="bg-pepe-black text-white p-6 sm:p-8 rounded-[2rem] border-4 border-pepe-black shadow-[8px_8px_0_0_#FF69B4] space-y-4">
              <div className="flex items-center justify-between bg-white/10 p-4 rounded-xl border-2 border-white/20">
                ...
                <div className="bg-pepe-green text-pepe-black px-3 py-1 rounded-full text-xs font-black uppercase animate-pulse">
                  Active
                </div>
              </div>
    ...
    <div className="absolute inset-0 z-0">
      <picture>
        ...
        <img 
          ...
          alt="Why Buy BG" 
          className="w-full h-full object-cover lg:object-fill object-center bg-no-repeat brightness-[0.8] contrast-[1.2]" 
        />
      </picture>
    </div>
